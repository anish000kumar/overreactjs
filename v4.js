  ...
  render() {
    ...
    return `
              <div class="card">
                  ...
                  <div class="buttons"> 
                    <button onClick="Card.increase(event)"> + </button>
                    <button onClick="Card.decrease(event)"> - </button>
                  </div>
                  <h4>${props.name}</h4>
                  <div class="details">
                      <p> ${props.details} </p>
...
const App = createComponent({
  ...
  methods: {
    moveToWinners(name) {
      ...
      // add to winner.
      this.state.winners = this.state.winners.map((p) => p.name).includes(name)
        ? this.state.winners
              ...
              <div class="columns">
                  ...
                  <div class="box">
                      ...
                      ${state.winners
                        .map(
                          (card) => `
                          <div onClick="App.moveFromWinners('${card.name}')">
                              ${Card.withProps({
                                title: card.title,
                                name: card.name,
                                image: card.image,
